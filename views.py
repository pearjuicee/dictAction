...
def grading(err_count, word_count):
    ...
    """
    # Assuming err_count and word_count are obtained from the form
    err_count = int(request.form['err_count'])
    word_count = int(request.form['word_count'])
    percentage = int((word_count - err_count) / word_count * 100)  # This converts the result to an integer

    return render_template("CorrectionResult.html", grade=percentage)
